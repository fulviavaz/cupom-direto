    ...
    try {
        ...
        if (sessionCookie) {
            try {
                ...
                uploadedById = session?.userId ? Number(session.userId) : null
            } catch {
                uploadedById = null
            }
        }
        ...
        const couponRows = XLSX.utils.sheet_to_json<RawRow>(couponsSheet, {
            defval: '',
            raw: true,
        })
        ...
        for (const row of brandRows) {
            ...
            if (id && name && id.toLowerCase() !== 'gerado pelo sistema') {
                brandMap.set(id, row)
            }
        }
        ...
        for (const [index, row] of couponRows.entries()) {
            ...
            try {
                ...
                const code = getString(row['Código Cupom']) || null
                const description = getString(row['Descrição / regras de uso']) || null
                const redirectUrl = getString(row['Link afiliado']) || null

                const brandRef = getString(row['Marca'])
                const categoryRef = getString(row['Categoria'])

                const expiresAt = getDate(row['Validade cupom'])
                const isActive = getBoolean(row['Ativo ou não'])
                const priority = getNumber(row['Prioridade']) ?? 0

                if (!title || title.toLowerCase() === 'texto') {
                    continue
                }

                if (!brandRef) {
                    errors.push(`Linha ${line}: marca não informada`)
                    previewData.push({
                        title,
                        store: '',
                ...
                if (!categoryRef) {
                    errors.push(`Linha ${line}: categoria não informada`)
                    previewData.push({
                        title,
                        store: brandRef,
                        category: '',
                ...
                if (brandRow) {
                    storeName = getString(brandRow['Marca']) || brandRef
                    storeDescription = getString(brandRow['Descrição Marca']) || null
                    storeLogoUrl = getString(brandRow['Logo Marca']) || null
                    storeIsActive = getBoolean(brandRow['Ativo ou não'])
                    storeIsFeatured = (getNumber(brandRow['Prioridade']) ?? 999) === 0
                }
                ...
                const categoryRow = categoryMap.get(categoryRef)
                if (categoryRow) {
                    categoryName = getString(categoryRow['Categoria']) || categoryRef
                    categoryIsActive = getBoolean(categoryRow['Ativo ou não'])
                    categoryIsFeatured = (getNumber(categoryRow['Prioridade']) ?? 999) === 0
                }

                if (!storeName) {
                    errors.push(`Linha ${line}: não foi possível resolver a marca`)
                    previewData.push({
                        title,
                        store: brandRef,
                ...
                if (!categoryName) {
                    errors.push(`Linha ${line}: não foi possível resolver a categoria`)
                    previewData.push({
                        title,
                        store: storeName,
                        category: categoryRef,
                ...
                let category = await prisma.tag.findFirst({
                    where: {
                        name: {
                            equals: categoryName,
                        },
        ...
        const totalRows = couponRows.filter((row) => {
            const title = getString(row['Título Cupom'])
            return !!title && title.toLowerCase() !== 'texto'
        }).length
